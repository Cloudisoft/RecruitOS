...
export function TableSkeleton({ rows = 6, cols = 5 }: { rows?: number; cols?: number }) {
  return (
    <div className="animate-pulse divide-y divide-[#e5e7eb]">
      {Array.from({ length: rows }).map((_, r) => (
        <div key={r} className="flex gap-4 px-5 py-3">
          {Array.from({ length: cols }).map((_, c) => (
            <div key={c} className="h-4 flex-1 rounded bg-[#e5e7eb]" />
          ))}
        </div>
      ))}
    </div>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 px-6 py-16 text-center">
      <div className="mb-2 rounded-full bg-[#f1f5f9] p-3 text-gray-500">{icon ?? <Inbox className="h-6 w-6" />}</div>
