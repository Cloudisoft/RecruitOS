...
matchingRouter.post('/explain', async (req: AuthedRequest, res) => {
  if (!isAIConfigured) {
    ...
    res.status(503).json({ error: `AI-enhanced matching is not configured on this server. Set ${envVar}. Rule-based matching still works without it.` })
    return
  }
  ...
  try {
    const result = await completeJson(SYSTEM_PROMPT, `Job:\n${JSON.stringify(job)}\n\nShortlisted candidates:\n${JSON.stringify(candidates)}`)
    res.json(result)
  } catch (e: any) {
